public class TicTacToeBoard implements GeneralBoard {
    protected Tile[][] grid;
    protected int checkLength;

    // Initialize the general board with numbers
    public TicTacToeBoard(int size, int checkLength) {
    ...
    public boolean isValidMove(int pos, int startY, int endX, int endY, Player[] players) {
    ...
    public void makeMove(int pos, int dummyY, Piece piece) {
        int row = (pos - 1) / grid.length;
        int col = (pos - 1) % grid.length;
